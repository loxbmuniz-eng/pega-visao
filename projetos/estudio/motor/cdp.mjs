...
//
// POR QUE sem biblioteca: o Node 22 já traz WebSocket nativo, e o único
// recurso do Chrome que o estúdio usa é "carrega, avança o relógio, tira
// print". Puppeteer/Playwright trariam ~300 MB e uma versão de navegador
// para gerenciar em troca de nada que precisamos. Mesma escolha do painel
// Suinco no frontend: menos peça, menos coisa para quebrar.
...
export async function abrirNavegador({ binario, argsExtra = [] } = {}) {
  ...
  const perfil = await mkdtemp(join(tmpdir(), 'estudio-perfil-'));
  const args = [
    '--headless=new',
    ...
    '--disable-lcd-text',                 // subpixel muda por máquina: quadro deixaria de ser reprodutível
