...
async def main():
    async with async_playwright() as p:
        ...
        # O marcador é informação; o alvo de toque é um botão na expansão (regra dos 44px do celular).
        chip = await pg.evaluate("() => { const e = document.querySelector('.marca-frota'); return e ? { onclick: e.getAttribute('onclick') } : null; }")
        ck('o marcador não é botão (não vira alvo de toque de 17px)', chip is not None and not chip['onclick'], str(chip))
        await pg.evaluate("abrirTab('programacao'); alternarLinhaProgFilaUI('c_div')")
        await pg.wait_for_timeout(400)
        botao = await pg.evaluate("() => { const b = [...document.querySelectorAll('#prog-fila-tbody button')].find(x => /usar a da Frota/i.test(x.textContent)); return b ? b.textContent.trim() : null; }")
        ck('na carga aberta há o botão "usar a da Frota"', botao is not None and 'Denia' in (botao or ''), str(botao))
        await pg.evaluate("abrirTab('torre')")
